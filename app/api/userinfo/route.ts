...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check cache first
    const cachedData = CacheService.getCachedData(token, "userinfo")
    if (cachedData) {
    ...
    console.log("Cache MISS for userinfo - fetching fresh data")
    const fetch = await fetchUserInfo(token)
    if (fetch.error) {
    ...
    CacheService.setCachedData(token, "userinfo", parse)
