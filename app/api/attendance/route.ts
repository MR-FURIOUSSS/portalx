...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check cache first
    const cachedData = CacheService.getCachedData(token, "attendance")
    if (cachedData) {
    ...
    console.log("Cache MISS for attendance - fetching fresh data")
    ...
    CacheService.setCachedData(token, "attendance", parse)
